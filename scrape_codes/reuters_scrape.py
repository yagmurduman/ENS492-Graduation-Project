...
CATEGORIES = {'economy': 'https://www.reuters.com/news/archive/economicNews',
            'world': 'https://www.reuters.com/news/archive/worldnews',
...
def find_urls(url_subject):
    ...
    while count < 120:
        try:
            loadMoreButton = driver.find_element_by_xpath("//a[@class='control-nav-next']")
            loadMoreButton.click()
            time.sleep(2)

            soup = BeautifulSoup(driver.page_source, 'html.parser')           
            div = soup.find("div", attrs={"class": "news-headline-list"})

            for article in div.find_all("div", attrs={"class": "story-content"}):
    ...
    for url in urls:
        ...
        if (response.status_code // 100 == 2):
            ...
            try:
                ...
                texts = ""
                section = soup.find("div", attrs={"class": "ArticleBodyWrapper"})
                if section == None:
                    section = soup.find("div", attrs={"class": "ArticleBody__container___D-h4BJ"})
                    pars = section.find_all("div", attrs={"class": "Text__text___3eVx1j Text__dark-grey___AS2I_p Text__regular___Bh17t- Text__large___1i0u1F Body__base___25kqPt Body__large_body___3g04wK ArticleBody__element___3UrnEs"})
                else:
                 pars = section.find_all("p", attrs={"class": "Paragraph-paragraph-2Bgue"})
                for par in pars:
                ...
                # imageFile = search_google(keywords)
...
def main(date):
    ...
    for cat in CATEGORIES:
        fileName =  cat + "_reuters_" + str(date) + ".csv"
